...
func Load() (Config, error) {
	...
	maxID, err := strconv.Atoi(value("CONTENT_VOA_MAX_ARTICLE_ID", "0"))
	if err != nil {
		return Config{}, fmt.Errorf("CONTENT_VOA_MAX_ARTICLE_ID: %w", err)
	}
	c := Config{
		Sitemap:   value("CONTENT_SOURCE_SITEMAP", "https://learningenglish.voanews.com/sitemap.xml"),
		UserAgent: value("CONTENT_USER_AGENT", "EnglishLearningContentLibrary/0.1 (+https://example.com/contact)"),
